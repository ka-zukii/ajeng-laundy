<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="mt-4">
            ...
            <x-text-input id="phone" class="block mt-1 w-full" type="tel" name="phone" :value="old('phone')" required autocomplete="tel" placeholder="Enter Your Phone Number" />
        ...
        <div class="mt-4">
            ...
            <x-text-input id="address" class="block mt-1 w-full" type="text" name="address" :value="old('address')" required placeholder="Enter Your Address" />
            <x-input-error :messages="$errors->get('address')" class="mt-2" />
        </div>

        <!-- Submit Button (Pink Soft) -->
        <div class="mt-6">
            <button type="submit" class="w-full py-3 px-4 bg-[#FFB7C5] hover:bg-[#ff9eae] text-white font-semibold rounded-md shadow-sm transition duration-150 ease-in-out text-center">
                {{ __('Log In') }}
            </button>
        </div>
    </form>
